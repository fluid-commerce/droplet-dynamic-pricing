...
describe Webhooks::SubscriptionPausedController do
  fixtures(:companies)
  ...
  describe "POST #create" do
    it "returns success when webhook is processed successfully" do
      Webhooks::SubscriptionPausedService.stub :call,
{ success: true, message: "Subscription paused webhook processed successfully" } do
        post webhook_subscription_paused_path, params: @webhook_params,
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json

        _(response.status).must_equal 200
        json_response = JSON.parse(response.body)
        _(json_response["success"]).must_equal true
        _(json_response["message"]).must_equal "Subscription paused webhook processed successfully"
      end
    end
...
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "returns internal_server_error when service raises an exception" do
      Webhooks::SubscriptionPausedService.stub :call, ->(*args) { raise StandardError, "Unexpected error" } do
        post webhook_subscription_paused_path, params: @webhook_params,
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "requires valid authentication token" do
      post webhook_subscription_paused_path, params: @webhook_params, as: :json
    ...
    it "accepts company webhook verification token" do
      Webhooks::SubscriptionPausedService.stub :call, { success: true, message: "Success" } do
        post webhook_subscription_paused_path, params: @webhook_params,
...
headers: { "X-Auth-Token" => @webhook_auth_token }, as: :json
    ...
    it "returns not_found when company is not found" do
      ...
      post webhook_subscription_paused_path, params: invalid_params, headers: { "AUTH_TOKEN" => @webhook_auth_token },
as: :json
    ...
    it "calls service with correct parameters" do
      ...
      Webhooks::SubscriptionPausedService.stub :call, ->(params, company) {
        service_called = true
