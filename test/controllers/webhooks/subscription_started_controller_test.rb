...
describe Webhooks::SubscriptionStartedController do
  fixtures(:companies)
  ...
  describe "POST #create" do
    it "returns success when webhook is processed successfully" do
      Webhooks::SubscriptionStartedService.stub :call,
{ success: true, message: "Subscription started webhook processed successfully" } do
        post webhook_subscription_started_path, params: @webhook_params,
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json

        _(response.status).must_equal 200
        json_response = JSON.parse(response.body)
        _(json_response["success"]).must_equal true
        _(json_response["message"]).must_equal "Subscription started webhook processed successfully"
      end
    end
...
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "returns internal_server_error when service raises an exception" do
      Webhooks::SubscriptionStartedService.stub :call, ->(*args) { raise StandardError, "Unexpected error" } do
        post webhook_subscription_started_path, params: @webhook_params,
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "requires valid authentication token" do
      post webhook_subscription_started_path, params: @webhook_params, as: :json
...
headers: { "X-Auth-Token" => @webhook_auth_token }, as: :json
    ...
    it "returns not_found when company is not found" do
      ...
      post webhook_subscription_started_path, params: invalid_params, headers: { "AUTH_TOKEN" => @webhook_auth_token },
as: :json
    ...
    it "calls service with correct parameters" do
      ...
      Webhooks::SubscriptionStartedService.stub :call, ->(params, company) {
        service_called = true
