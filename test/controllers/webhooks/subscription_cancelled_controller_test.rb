...
describe Webhooks::SubscriptionCancelledController do
  fixtures(:companies)
  ...
  describe "POST #create" do
    it "returns success when webhook is processed successfully" do
      Webhooks::SubscriptionCancelledService.stub :call,
{ success: true, message: "Subscription cancelled webhook processed successfully" } do
        post webhook_subscription_cancelled_path, params: @webhook_params,
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json

        _(response.status).must_equal 200
        json_response = JSON.parse(response.body)
        _(json_response["success"]).must_equal true
        _(json_response["message"]).must_equal "Subscription cancelled webhook processed successfully"
      end
    end
...
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "returns internal_server_error when service raises an exception" do
      Webhooks::SubscriptionCancelledService.stub :call, ->(*args) { raise StandardError, "Unexpected error" } do
        post webhook_subscription_cancelled_path, params: @webhook_params,
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "requires valid authentication token" do
      post webhook_subscription_cancelled_path, params: @webhook_params, as: :json
    ...
    it "accepts company webhook verification token" do
      Webhooks::SubscriptionCancelledService.stub :call, { success: true, message: "Success" } do
        post webhook_subscription_cancelled_path, params: @webhook_params,
...
headers: { "AUTH_TOKEN" => @webhook_auth_token }, as: :json
    ...
    it "calls service with correct parameters" do
      ...
      Webhooks::SubscriptionCancelledService.stub :call, ->(params, company) {
        service_called = true
