...
# STU2-3108. The droplet's own price_locked stamp is what strands a cart line in
# the country it was written for; this callback is the signal that lets the
# writer correct it without releasing the lock.
class Callbacks::CartCountryChangedServiceTest < ActiveSupport::TestCase
  ...
  # A PH cart holding the figure a previous write locked while the cart was in
  # US. Core has already cleared ship_to and repriced the unlocked lines, so
  # subscription_price is the new country's number and item.price is the stale
  # one — exactly the shape the droplet has to close.
  ...
  def test_reasserts_the_preferred_customer_slug_so_the_price_type_cannot_drift
    # The line price and the price_type slug travel on separate channels; a
    # reprice that leaves the slug behind produces an order priced as preferred
    # but labelled retail. Same reasoning as CartItemAddedService.
    service = Callbacks::CartCountryChangedService.new(callback_params)
    carts = Object.new
  ...
  def test_leaves_a_cart_that_was_never_preferred_alone
    # No preferred pricing means this droplet never wrote — and never locked — a
    # line on this cart, so core's own reprice is already right. Writing here
    # would lock a price Fluid set itself.
    carts = Object.new
  ...
  def test_records_the_reprice_as_a_country_changed_pricing_event
    ...
    # from the enum degrades to a log line and nothing else. Caught live, pinned
    # here: the dashboard is the only record that this repair ran.
    carts = Object.new
    carts.define_singleton_method(:update_items_prices) { |_token, _items| { "success" => true } }
    carts.define_singleton_method(:append_metadata) { |_token, _metadata| { "success" => true } }
  ...
  def test_refreshes_volumes_for_the_new_country_when_the_company_opts_in
    # CV/QV carry their own lock — Fluid's update_volumes stamps
    # cv_manually_updated — so the volumes written under the old country stay put
    # unless this callback refreshes them alongside the prices.
