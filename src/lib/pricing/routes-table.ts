 ...
 *
 * Some Rails routes were named for something OTHER than their Fluid
 * definition (`/callbacks/subscription_added` serves `cart_subscription_added`),
 * because the Rails path was a LOCAL name an operator typed into the admin Callbacks
 ...
 *
 * `cart_customer_logged_in` is not served. Core fires it only on magic-link
 * login, and the same action fires `cart_customer_attached` right after, which
 * reprices the same cart.
 */
...
/** The eight definition names, in the order CUTOVER.md lists them. */
 ...
 * direction.
 *
 * One differs from the definition name, and that asymmetry is the whole
 * reason this table exists rather than a string transform:
 * `cart_subscription_added` is served at `/callbacks/subscription_added`. A
 * rollback that derived the Rails path from the definition name would register
 * it at a route Rails does not have — and because Fluid discards the status of
 * most callbacks, the symptom would be "prices are wrong", not an error.
 */
