...
const nextConfig: NextConfig = {
  ...
  // `mssql` (and the tedious driver under it) is required at runtime from
  // node_modules rather than bundled. It is a TDS client full of conditional
  // requires and node built-ins, and webpack's static analysis of those is the
  // kind of thing that builds cleanly and then throws on the first Exigo query
  // in production — which on the callback path fails to "unknown preferred
  // status", i.e. silently. File tracing still pulls it into the standalone
  // output, so the image is unaffected.
