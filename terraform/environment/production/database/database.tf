# fluid droplet dynamic pricing - database configuration
locals {
  application_name = "fluid-droplet-dynamic-pricing"
  postgres_name_database = "dynamic_pricing"
  database_user_name = "dynamic_pricing_user"
  secret_name_database_url_prefix = "DYNAMIC_PRICING"
}
...
resource "google_secret_manager_secret" "database_url" {
  ...
  annotations = {
    description = "URL for ${local.application_name} PostgreSQL database for ${local.application_name} application instance named fluid-studioz"
...
resource "google_secret_manager_secret" "queue_database_url" {
  ...
  annotations = {
    description = "URL for ${local.application_name} Queue PostgreSQL database for ${local.application_name} application instance named fluid-studioz"
    created-by  = "terraform"
    service     = "${local.application_name}"
  }
}
...
resource "google_secret_manager_secret" "cable_database_url" {
  ...
  annotations = {
    description = "URL for ${local.application_name} Cable PostgreSQL database for ${local.application_name} application instance named fluid-studioz"
    created-by  = "terraform"
    service     = "${local.application_name}"
  }
}
